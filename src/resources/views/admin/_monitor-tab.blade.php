{{--
    Partial partagé pour l'onglet "Log / Monitoring" des pages Paramètres
    de chaque section (Sites, Balises, Stations météo).

    Variables attendues :
      $monitorSummary  — array job_class => { label, schedule, last, total_24h, ... }
      $monitorEntries  — LengthAwarePaginator de JobMonitor
      $monitorGroup    — string (sites|balises|stations)
      $monitorJob      — ?string (filtre par classe)
      $monitorStatus   — ?string (filtre par statut)
      $baseRoute       — string (route de base pour les filtres)
--}}
...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
    @foreach ($monitorSummary as $class => $s)
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-4">
            ...
            <div class="mt-2">
                <a href="{{ route($baseRoute, ['tab' => 'logs', 'job' => $class]) }}"
...
<div class="flex items-center gap-3 mb-4">
    <form method="GET" action="{{ route($baseRoute) }}" class="flex items-center gap-2 flex-wrap">
        ...
        <select name="job" onchange="this.form.submit()"
            ...
            @foreach ($monitorSummary as $class => $s)
                <option value="{{ $class }}" @selected($monitorJob === $class)>
                    {{ $s['label'] }}
                </option>
            @endforeach
        </select>
        <select name="status" onchange="this.form.submit()"
            ...
            <option value="success" @selected($monitorStatus === 'success')>Succès</option>
            <option value="failed" @selected($monitorStatus === 'failed')>Échecs</option>
            <option value="running" @selected($monitorStatus === 'running')>En cours</option>
        </select>
        @if ($monitorJob || $monitorStatus)
            <a href="{{ route($baseRoute, ['tab' => 'logs']) }}" class="text-xs text-gray-400 hover:text-white transition">
...
@if ($monitorEntries->isEmpty())
    <x-admin.empty-state icon="fa-solid fa-clipboard-list" message="Aucune exécution enregistrée. Les données apparaîtront après la prochaine exécution des jobs planifiés." />
@else
    <div class="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-800" x-data="{ expanded: null }">
                @foreach ($monitorEntries as $entry)
                    ...
                    <tr x-show="expanded === {{ $entry->id }}" x-cloak class="bg-gray-950/50">
                        <td colspan="5" class="px-6 py-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                                <div>
                                    <div class="text-gray-500 uppercase tracking-wider mb-1">Classe</div>
