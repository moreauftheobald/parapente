...
<div>
    ...
    {{-- Inliné (pas de partial) pour rester un fichier unique et éviter
         qu'un nouveau fichier soit perdu lors d'un merge manuel. --}}
    <h2 class="text-xs uppercase tracking-wider text-gray-400 mb-3">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <i class="fa-solid fa-tower-broadcast w-4 text-center" style="color:{{ $api->networkColor() }}"></i>
                                <div>
                                    <div class="text-gray-100 font-medium">{{ $api->networkLabel() }}</div>
                                    <div class="text-xs text-gray-500 font-mono">{{ $api->code }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <div class="inline-flex items-center gap-1">
                                <a href="{{ route('admin.station-apis.edit', $api) }}"
                                   class="w-8 h-8 inline-flex items-center justify-center rounded border border-gray-700 text-gray-300 hover:bg-gray-800 hover:text-white transition"
                                   title="Éditer">
                                    <i class="fa-solid fa-pen text-xs"></i>
                                </a>
                                <form method="POST" action="{{ route('admin.station-apis.toggle', $api) }}" class="inline">
                                    @csrf
                                    <button type="submit"
                                            class="w-8 h-8 inline-flex items-center justify-center rounded border transition
                                                @class([
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-0 py-0">
                            <x-admin.empty-state icon="fa-solid fa-plug" message="Aucune API configurée. Lancez le seeder StationApiSeeder." class="border-0 rounded-none" />
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <p class="text-xs text-gray-500 mt-4">
        <i class="fa-solid fa-circle-info"></i>
        Les APIs stations sont créées par le <code class="font-mono text-gray-400">StationApiSeeder</code>.
